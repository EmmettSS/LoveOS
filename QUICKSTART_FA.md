...
* اگر می‌خواهی بدانی همه‌چیز سالم است: `python manage.py test` (۹۷ تست) و در پوشه‌ی فرانت‌اند
  `npx tsc -b && npx oxlint src && npm run build && npm run test:ui` (۱۰ سوئیت، ۳۲۶ بررسی)
...
| انیمیشن‌ها کندند یا گوشی داغ می‌کند | لایه‌ی سه‌بعدی از توانِ دستگاه بالاتر است | در اپ «تنظیمات ← جلوه‌های سه‌بعدی» بگذار روی **مهتاب**؛ سیستم خودش هم بعد از دو بار افتِ فریم یک پله پایین می‌آید و پیام می‌دهد |
| روی نقشه، کره‌ی زمین ظاهر نمی‌شود | عمدی است | کره فقط در لایه‌ی **کهکشان** و فقط روی GPU غیرِ Mali روشن می‌شود (باگِ شناخته‌شده‌ی دقتِ مختصات در maplibre). دکمه‌ی «کره/تخت» گوشه‌ی نقشه همیشه هست |
| ستاره‌های آسمان سه‌بعدی نمی‌چرخند | لایه‌ی فعلی مهتاب یا بلور است | WebGL فقط در لایه‌ی **کهکشان** فعال است؛ در لایه‌های پایین‌تر همان ستاره‌ها به‌صورت SVG سبک رندر می‌شوند |
