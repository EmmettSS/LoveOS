...
python manage.py check          # باید بگوید: System check identified no issues
```

> از نسخه‌ی ۲.۰ پنج اپ تازه (تماس، هدیه‌ها، کتاب‌خوانی مشترک، خونه‌ی رویایی، پل زبان) هم مهاجرت
> دارند؛ اگر دیتابیس قدیمی داری فقط `migrate` را بزن، خودش همه را می‌سازد. `seed_loveos` هم برای
> اپ‌های تازه نمونه‌ی واقعی می‌سازد: بازه‌های آزاد تماس، چند هدیه، دو کتاب با فصل و یادداشت،
> اتاق‌های خونه و کلمه‌های پل زبان — پس رابط بلافاصله پر است و می‌شود با خیال راحت تستش کرد.
...
* اگر می‌خواهی بدانی همه‌چیز سالم است: `python manage.py test` (۸۴ تست) و در پوشه‌ی فرانت‌اند
  `npx tsc -b && npx oxlint src && npm run build && npm run test:ui` (۷۴+ بررسی رابط کاربری)
...
| اپ باز می‌شود ولی سفید است | کش قدیمی سرویس‌ورکر | در مرورگر یک بار `Ctrl + Shift + R`؛ نسخه‌ی ۲.۰ خودش سرویس‌ورکر را به‌روز می‌کند |
| دکمه‌ی بستن پنجره کار نمی‌کند | نسخه‌ی قدیمی فرانت‌اند در حال اجراست | Vite را ری‌استارت کن و مطمئن شو `frontend/src/os/Window.tsx` نسخه‌ی تازه است |
| موقعیت و ساعت قدیمی است | موقعیت زنده از گوشی نیامده | در اپ «تنظیمات» او، موقعیت زنده را روشن کن یا در پنل مقدار درست را بگذار |
