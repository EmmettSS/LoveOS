...
import { probeCapability } from '../shared/quality'
...
export function useThreeScene(
  ...
  useEffect(() => {
    ...
    if (minTier === 'dream' && report.webgl === 0) {
    ...
    const pixelRatio = pixelRatioFor('dream', opts.maxPixelRatio ?? 2)
    ...
    try {
      api = factoryRef.current({ scene, camera, renderer, width, height, tier: 'dream', pixelRatio })
    } catch {
      api = null
    }
    if (!api) {
    ...
    const doResize = () => {
      ...
      if (!renderer) return
      renderer.setPixelRatio(pixelRatioFor('dream', opts.maxPixelRatio ?? 2))
