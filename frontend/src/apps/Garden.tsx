...
function Plant({ f, bloom }: { f: Flower; bloom: boolean }) {
  ...
  return (
    <svg width="88" height="120" viewBox="0 0 88 120" className="mx-auto">
      ...
      />
      {/* برگ‌ها */}
      {s >= 2 && <ellipse cx="34" cy={92 - height * 0.45} rx="9" ry="5" fill="#7ccb80" transform={`rotate(-25 34 ${92 - height * 0.45})`} />}
      {s >= 3 && <ellipse cx="54" cy={92 - height * 0.62} rx="9" ry="5" fill="#7ccb80" transform={`rotate(25 54 ${92 - height * 0.62})`} />}
      {/* گل */}
      {s >= 4 && (
        <motion.g
          initial={{ scale: 0 }}
...
export default function Garden() {
  ...
  const water = async (f: Flower) => {
    ...
    setData((prev) => (prev ? { items: prev.items.map((x) => (x.id === f.id ? { ...x, water_count: res.water_count } : x)) } : prev))
    // راز ⑪ — پنج بار آب دادن به یک گل
      ...
      <div className="grid grid-cols-2 gap-3">
        {items.map((f, i) => (
          ...
          >
            ...
            <p className="os-title mt-1 text-sm">{f.emoji} {f.name}</p>
