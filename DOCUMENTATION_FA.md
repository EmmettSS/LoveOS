...
| اپ | چه می‌کند |
|---|---|
| **نقشه‌ی ما** | کره‌ی واقعی زمین (قابل چرخش و زوم، با فضای ستاره‌ای و هاله‌ی جو) که قابِ استراحتش **تمامِ ایران از دریای خزر تا خلیج فارس** است؛ دو قلب روی کره، نخ قوسی درخشان، فاصله و اختلاف ساعت و نشان موقعیت زنده. دکمه‌ی «پرواز روی مسیر» دنباله‌ی سینماییِ ~۲۰ ثانیه‌ای و غیرقابل‌رد می‌سازد: پرده‌ی سینما و عقب‌کشی به کره‌ی کامل با شهاب و شکوفاییِ هاله‌ی جو، شیرجه به خانه‌ی بابا با مدار و مکث، بلند شدن با هوووش، پروازِ آهسته روی کمان با قلبِ جرقه‌ای و ردپای قلب‌های محوشونده و شمارنده‌ی زنده‌ی کیلومتر و کمانِ نور، فرودِ جشن‌گونه‌ی چهارموجی با پیام پایانی، و بازگشتِ نرم به همان قابِ ایران. نخ در همه‌ی سکانس همگام با ضربانِ قلب می‌تپد (نور و صدا با هم) و زیرنویس‌های کوتاهِ دوزبانه روی نقشه می‌آید. سقفِ زومِ سکانس ۱۱٫۶ است تا راز ⑨ تصادفی مصرف نشود؛ ریاضیِ قاب و کل زمان‌بندی در `mapOfUsCinema.ts` است و با آزمونِ jsdom سنجیده می‌شود. |
...
| **حال دلم** | حال امروز را ثبت کند و پیامی متناسب از من بگیرد. حال‌های سفارشی هم دارد: با ایموجی و اسمِ دلخواه خودش «حال تازه» می‌سازد (و می‌تواند یادداشت بنویسد)؛ همان لحظه در سرور ذخیره می‌شود، در فهرستِ حال‌هایش می‌ماند و برای من (پنل + سروش) خبر می‌رود. اگر سرور در دسترس نباشد، به‌جای سکوت پیامِ روشن با «دوباره امتحان کن» می‌بیند. |
...
| **خونه‌ی رویایی** | چک‌لیست با درجه‌ی اهمیت، چیدمان اتاق‌ها (ضربه = ویرایش، نگه‌داشتن یا اتاقِ انتخاب‌شده = جابه‌جایی)، گالری الهام با نظر. |
...
| **آماده‌سازی و مجوزها** | بعد از قفل و قبل از دسکتاپ، صفحه‌ی «آماده‌سازی LoveOS» همه‌ی اجازه‌ها را با لمسِ خودش می‌گیرد: تمام‌صفحه، اعلان، موقعیت، میکروفن، دوربین و نگه‌داشتنِ حافظه. «بعداً» فقط تا پایانِ همان باز بودنِ اپ آرام می‌کند؛ هر بار که اپ باز شود و اجازه‌ای کم باشد دوباره می‌پرسد. تمام‌صفحه روی گوشی با هر لمس دوباره گرفته می‌شود تا نوارهای مرورگر برنگردند، و جایی که مرورگر پشتیبانی نمی‌کند (آیفون) پیشنهادِ «افزودن به صفحه‌ی خانه» می‌آید. همه‌ی این‌ها از «تنظیمات ← مجوزها» دستی هم قابل تغییرند. |
| **اعلانِ گوشی** | خبرِ تازه (نامه، ویس، یادآور) در حالی که اپ در پس‌زمینه است، به‌عنوان اعلانِ واقعیِ گوشی هم نشان داده می‌شود (از سرویس‌ورکر؛ روی اندروید همان راهِ درست است) و با ضربه روی آن، LoveOS باز/فوکوس می‌شود. |
...
| `test:be` | `manage.py check` + `manage.py test` (۱۰۲ تست) |
...
فلش‌کارت/زنجیره‌ی زبان و کوییز فقط از پنل، «همه‌ی مدل‌های ثبت‌شده در ادمین رندر می‌شوند»، و
ترمیمِ دیتابیس‌های قدیمی (ستونِ یتیمِ `NOT NULL` مثلِ `content_moodlog.note`، درمانِ خودکار در زمان
اجرا، و یک مسیرِ کاملِ دیتابیسِ قدیمی ← `migrate` ← `seed`).
...
| `starmap-sky` | آسمان لبه‌به‌لبه با ۸ صورت فلکی واقعی، زدن → پیام، «کل اسم رو روشن کن» |
...
| `mapofus-iran-cinema` | قابِ ایران و سکانسِ سینماییِ ~۲۰ ثانیه‌ای (ریاضی و ناحیه‌بندی) |
| `boot-timeout` | بک‌اندِ بی‌پاسخ از «در حال اتصال» به «دوباره تلاش» می‌رسد |
| `error-boundary-wrapper` | اپِ خراب فقط در پنجره‌ی خودش پوسته‌ی ملایمِ تلاش دوباره را نشان می‌دهد |
| `dreamhome-map-gestures` | ژست‌های تبِ نقشه در «خونه‌ی رویایی» |
| `setup-permissions` | صفحه‌ی آماده‌سازی هر هفت اجازه را به‌ترتیب می‌گیرد، «بعداً» فقط همین نشست را آرام می‌کند، و اجازه‌ای که مرورگر رد کرده دیگر پرسیده نمی‌شود |
| `language-bridge-quiz` | کارنامه‌ی کوییز بعد از تازه‌شدنِ آمار روی صفحه می‌ماند و «یه دور دیگه» کوییزِ تازه می‌آورد |
| `mood-custom` | ساختِ حالِ تازه (ایموجی + اسم + یادداشت)، رسیدنش به سرور و ماندنش در فهرست، و پیامِ روشن به‌جای سکوت وقتی سرور خطا می‌دهد |
| `phone-notifications` | خبرِ تازه در پس‌زمینه به اعلانِ واقعیِ گوشی تبدیل می‌شود (از سرویس‌ورکر) — و خبرهای دیده‌شده دوباره اعلان نمی‌شوند |
